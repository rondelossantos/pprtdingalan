<div
    ...
    id="voidItemModal"
    tabindex="-1"
    aria-labelledby="voidItemModalTitle"
    aria-modal="true"
    role="dialog"
    >
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="void-item-form" action="{{ route('order.void_item') }}" method="post" autocomplete="off">
                @csrf
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to void this order item (name: <span x-text="$store.item.void.name" class="font-bold"></span>)? Voiding item will <b>REMOVE</b> it in kitchen, dispatch, or production orders page.</p>
                    <input name="id" type="hidden" :value="$store.item.void.id">
                </label>
            </form>
        </div>
        <div
