<div
    ...
    <div class="relative w-auto pointer-events-none modal-dialog modal-dialog-centered">
        ...
        <div class="relative p-4 modal-body">
            <form id="delete-expense-form" action="{{ route('expense.report.delete') }}" method="post">
                @csrf
                @method('delete')
                <label class="block mb-4 text-sm">
                    <p>Are you sure you want to delete this item (name: <span x-text="$store.expense.data?.name"></span>)?</p>
                    <input name="id" type="hidden" :value="$store.expense.data?.id">
                </label>
            </form>
        </div>
        <div
